...
for (ff in 1:length(inFiles))
{
  ...
  for (rr in 1:dim(tmp)[1])
  {
    ...
    tempDet2$HumanVoices[1] =   tmp2[11]
    tempDet2$HumanActivity[1] = tmp2[12]
    ...
    tempDet2$HumanVoices[2] =   tmp2[23]
    tempDet2$HumanActivity[2] = tmp2[24]
    ...
    tempDet2$HumanVoices[3] =   tmp2[35]
    tempDet2$HumanActivity[3] = tmp2[36]
    ...
    tempDet2$HumanVoices[4] =   tmp2[47]
    tempDet2$HumanActivity[4] = tmp2[48]
    ...
    tempDet2$HumanVoices[5] =   tmp2[59]
    tempDet2$HumanActivity[5] = tmp2[60]
...
dataC$HumanActivity = sapply(strsplit(as.character(dataC$HumanActivity),","), `[`, 1)
#as.character(unique(dataC$HumanActivity))
dataC$Mammal = sapply(strsplit(as.character(dataC$Mammal),","), `[`, 1)
#as.character(unique(dataC$Mammal))
dataC$HumanVoices = sapply(strsplit(as.character(dataC$HumanVoices),","), `[`, 1)
#as.character(unique(dataC$HumanVoices))
...
for (ss in 1:length(sites)){ # ss = 8 #site
  
  #combine data for analysis/graphic
  tmp =  inData[inData$Site == sites[ss], ]
  tmp2 = tmp[,c('Period','Bird','Mammal','HumanVoices','HumanActivity',"Weather",'Unk','Static','Insect')]
  ...
  { 
    ...
    { 
      ...
      tmpAll2 =  tmp[-c(1,7,8), ]
      tmpAll2$type <-factor( tmpAll2$type, levels=c("HumanVoices","HumanActivity","Mammal","Bird","Insect","Weather"))
      #scale_x_discrete( axis_order=c("HumanVoices","HumanActivity","Mammal","Bird","Insect","Weather"))
      #positions <- c("Bird", "Insect", "Mammal","Voices","Activity")
      
      #------------------------------------------------
      #GRAPHICS
      #------------------------------------------------
      # 1) widlife response by site
      ...
      #-----
      # 3) All results on same graphic
      pALL = ggplot(data=tmpAll2, aes(y=value, x=type, fill=type) ) +
        geom_bar(stat="identity") +  geom_hline(yintercept = 0) +
        geom_errorbar(data = tmpAll2, aes(ymin=value-SE, ymax=value+SE),
                      width=.2, # Width of the error bars
                      position = position_dodge(.9)) +
        labs(title = "",
             caption = paste( "B. LISTENING RESULTS (N=",eobs, " eclipse, ", "N=", nobs, " non on ",ndys, " day[s])",sep = ""),
             x = "", y = "") +
        #title =   paste("Figure 3B: Listening results for ", substr(sites[ss],1,4),sep="")
        theme_classic(base_size = 12) +
        theme(legend.position="top") +
        theme(axis.text.x = element_text(angle = 45, hjust = 1))+
        scale_y_continuous(breaks=c(-4,-2,0,2,4), labels=c("","less", "no change","more", ""),limits=c(-4, 4)) 
      
      pALL
      ggsave(pALL, file=paste(sites[ss], "_", ss,"_ListeningResultsALL.png",sep = ""), width = 14, height = 10, units = "cm",dpi = 300, bg = "transparent")
      
      #-----
      # First graph- no error bars!
